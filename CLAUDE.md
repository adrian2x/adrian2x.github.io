...
Run the full CI sequence locally and make sure it is clean. These are the exact steps CI runs, in order:

```sh
pnpm lint && pnpm format:check && pnpm build
```

`pnpm dev` does **not** typecheck. It will happily render a page that `astro check` rejects, so a passing dev server is not a green build. Type errors surface only in `pnpm build` (which runs `astro check` first). This bites hardest on `astro-paper.config.ts`: its feature flags are discriminated unions (e.g. `editPost` is `{ enabled: true; url: string } | { enabled: false }`), so a leftover field only fails at build, never in dev.

Pushing to `main` deploys to Pages, so a red build blocks the deploy. Never push a config or schema change without running `pnpm build` first.
