...
author: Adrian Cruz
pubDatetime: 2023-04-01T17:00:00Z
tags:
  - python
  - beginners
  - tutorial
  - getting-started
description: "Install Python, write your first programs, and learn variables, operators, and how to read errors - a beginner-friendly start to Python."
---
...
The Python programming language was actually named after Monty Python's Flying Circus TV show, not after the snake.

## Installing Python

To install Python, navigate to [python.org/downloads](https://www.python.org/downloads) and click on Download Python. Grab the latest Python 3.x release - any recent version is fine.

Once downloaded, double-click the file to run the installer and accept the defaults as you click through it. On Windows, make sure you tick **Add Python to PATH** on the first screen - that lets you run `python` from any terminal.

When the setup is complete, you're ready to go. On macOS, you'll find a Python folder inside your Applications folder.

Now that you have Python installed, we can write our first program.

### For Windows

Search for the Command Prompt in the Start Menu or the Windows search box. Enter `python` in all lowercase. You should see a message from Python like this:

```text
C:\> python
Python 3.13.1 [MSC v.1942 64 bit (AMD64)] on win32
...
Older Macs shipped with Python preinstalled, but recent versions of macOS no longer include it. If you run `python3` before installing anything, macOS may prompt you to install the Xcode Command Line Tools - which is why we installed Python from python.org above.
...
To leave the Python prompt, press Ctrl-D, or type `exit()` and press Enter.
...
```python
work_days = 5 * 52  # 5 working days a week, 52 weeks a year
vacation = 5 * 2 + 4  # two weeks of vacation plus 4 extra days
work_days_after_vacation = work_days - vacation
print(work_days_after_vacation)
```
...
Variable names cannot contain a space, so we use underscores to separate words.

Uppercase letters are allowed too, but there is no special language rule about them - it's a matter of convention. By convention, Python programmers keep everyday variable names lowercase. The style guide (PEP 8) reserves `UPPER_CASE` for constants and `CapWords` for class names, so sticking to lowercase for regular variables keeps your intent clear. Python is a case-sensitive language, which means that `today` is not the same as `Today`.
...
To create and save a new program, enter `idle` in the Windows search box, and select the IDLE entry for your Python version. On a Mac, open your Applications folder and open the Python folder to find IDLE.

IDLE (Integrated Development and Learning Environment) is a Python file editor which lets you write programs in Python.
...
Many people prefer PyCharm because it provides a more user friendly environment. It really works like a charm. You can use it to create new programs and to run saved programs directly from within PyCharm.
