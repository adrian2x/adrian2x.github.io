...
author: Adrian Cruz
pubDatetime: 2023-10-22T17:00:00Z
tags:
  - python
  - data-types
  - beginners
  - tutorial
description: "A tour of Python's built-in data types - numbers, strings, lists, dicts, sets and more - with idiomatic examples and the gotchas that trip up beginners."
---
...
The de-facto standard is to use double quotes for string values. Triple quotes are for strings that span multiple lines - Python has no multi-line comment syntax, so a triple-quoted string sitting on its own is often used as a docstring (documentation) rather than a real comment.
...
>>> vowels.add('a')  # 'a' is already present, so nothing changes
>>> vowels
{'e', 'u', 'a', 'o', 'i'}
>>> vowels.remove('u')
>>> vowels
...
We have discussed a few different types of containers like strings, lists, sets, and dictionaries. In python, we can iterate over the elements in a container using iterators. The iterator is a special type that represents the position in a sequence. Its main operation is `next()`, which hands back the next element; it also implements `__iter__` (returning itself), which is what lets you use it directly in a `for` loop. If you've worked with database cursors, iterators are a similar concept. Here's an example:
...
Python 3.15 adds a built-in `frozendict`, an immutable, hashable dictionary type ([PEP 814](https://peps.python.org/pep-0814/)). Until that release lands, there is no built-in `frozendict`, so if you need an immutable dictionary on an older version you can install the [frozendict package with pip](https://pypi.org/project/frozendict/).
