...
author: Adrian Cruz
pubDatetime: 2025-02-08T17:00:00Z
tags:
  - python
  - concurrency
  - multithreading
  - performance
description: "How threading works in Python: the GIL, Thread and ThreadPoolExecutor, locks and race conditions, and when threads actually help."
---

Python has shipped a `threading` module since version 1.5.2, but threads are often misunderstood and misused because of the limitations and capabilities of threads in Python. When used correctly, they can make your code run faster and more efficiently with very little effort.
...
One caveat up front: in the standard CPython interpreter, the Global Interpreter Lock (GIL) stops threads from running Python bytecode on more than one CPU core at a time. So threads speed up I/O-bound work - waiting on the network, disk, or another system - rather than CPU-bound computation. We'll cover the GIL and its exceptions in detail below.
...
A thread is like a separate mini-program that runs within the main program. Each thread has its own set of instructions to follow and its own call stack, but - unlike a separate process - it shares the same memory as the other threads in the program. That shared memory is what lets threads cooperate on a task easily, and it is also what makes race conditions possible.
...
The GIL is not a law of nature, though. Since Python 3.13, CPython also ships an experimental free-threaded (no-GIL) build introduced by [PEP 703](https://peps.python.org/pep-0703/), and [PEP 779](https://peps.python.org/pep-0779/) made that build officially supported - though still optional - in Python 3.14. On a free-threaded build, threads really can execute Python bytecode on multiple cores at once. The default build that most people still run has the GIL, so the rest of this guide assumes it is in place.
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        # Block for the result, which also re-raises any exception the task raised
        future.result()
```
...
If we don't specify `max_workers`, the executor picks a default for us. Since Python 3.8 that default is `min(32, os.cpu_count() + 4)`, which leaves some headroom for I/O-bound work without spawning an unbounded number of threads on machines with many cores.
...
It is also sometimes called a _promise_ or a _deferred_. It provides a context for the result of a task that may or may not be executing and a way of getting a result once it is available.
...
Run this example and you will see a race condition - the final count comes out well below what we asked for:
...
def increment_counter():
    ...
    for _ in range(1000):
        ...
        value = counter
        # Hand control to another thread while holding a stale value
        sleep(0)
        # Write back based on the value we read a moment ago
        counter = value + 1

# Create four threads that all hammer the same counter
...
# We asked for 4000 increments, but many of them are lost
print(counter)
```

_An example cause of a race condition_

Incrementing a shared variable is not atomic: it reads the value, adds one, then stores the result, and the interpreter can switch to another thread between those steps. The `sleep(0)` call makes that gap easy to see - it hands control to another thread while we are still holding the old value, so several threads read the same number and each writes back `value + 1`, clobbering each other's work. The result is reliably far below 4000. Without the `sleep(0)`, the GIL usually keeps a plain `counter += 1` together on current CPython, so the race is hard to trigger by hand - but it is still there, and it surfaces the moment a thread pauses for I/O in the middle of an update, or runs on a free-threaded build.
...
def process_request(request):
    ...
    # Read it back: each thread only ever sees the value it stored
    print(f"Processing {local_data.request}")
...
# signal each worker to stop, then wait for them to exit
for _ in threads:
    q.put(None)
...
conn.printFile(list(printers.keys())[0], '/path/to/file.pdf', 'Title', {})
```
...
In this case, we should experiment with using threading for the IO-bound tasks of our programs and processes or multiprocessing for the CPU-bound parts, then measure which split actually performs best for the workload.
